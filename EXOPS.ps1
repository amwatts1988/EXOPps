...
function Connect-EXOPSSession {
    ...
    try
    {
        ...
        $ModulePath = @(Get-ChildItem -Path "$($env:LOCALAPPDATA)\Apps\2.0" -Filter "Microsoft.Exchange.Management.ExoPowershellModule.manifest" -Recurse);
        $ExoPowershellModule = Join-Path $ModulePath[0].Directory.FullName "Microsoft.Exchange.Management.ExoPowershellModule.dll"
